<template>
	...
	<div class="affirm">填写并核对订单信息</div>
	<div class="confirm_order_container">
		<div class="receive_info" v-if="virtual.isVG == 1">
			...
			<div v-if="orderAddress.data.memberName" class="receive_info_content flex_row_start_center">
				<div style="width:100%" v-if="exhibition_show">
					<div class="content_left flex_column_between_start"
						:class="{city_start:current_address_index==index}"
						v-for="(item,index) in address_list.data" :key="index"
						@click="confirmChangeAddress(index)">
						<div class="city_left">
							<img src="../../assets/city.png" alt="" class="city_img">
							<div class="city_cont">
								<span>{{ item.memberName }}</span>
								<span>{{ item.telMobile }}</span>
								<span><text>{{ item.addressAll }}</text>{{ item.detailAddress }}</span>
							</div>
							<div class="default_city flex_row_center_center" v-if="item.isDefault==1">
								默认地址
							</div>
							<img src="../../assets/goods/check_marks.png" alt="" class="img_xuan"
				...
				<div class="content_left flex_column_between_start city_start" v-else>
					<div class="city_left">
						<img src="../../assets/city.png" alt="" class="city_img">
						<div class="city_cont">
							<span>{{ orderAddress.data.memberName }}</span>
							<span>{{ orderAddress.data.telMobile }}</span>
							<span><text>{{ orderAddress.data.addressAll }}</text>{{
								orderAddress.data.detailAddress }}</span>
						</div>
						<div class="default_city flex_row_center_center" v-if="orderAddress.data.isDefault==1">
							默认地址
						</div>
						<img src="../../assets/goods/check_marks.png" alt="" class="img_xuan">
					</div>
			...
			<div v-if="orderAddress.data.memberName" class="exhibition" @click="exhibition">
				{{exhibition_show ? '收起地址' : '全部地址'}}
				<img v-if="exhibition_show" src="../../assets/uparrow.png" alt="">
				<img v-else src="../../assets/downarrow.png" alt="">
			</div>
			<div v-else class="exhibition_none">暂无收货地址</div>
			<div class="xian">
				<p></p>
			</div>
				...
				<div class="pre_message_info">
					<table>
						<tr v-for="(item, index) in virtual.virtualPre" :key="index">
							<td>
								<div class="tag_pre"><strong v-if="item.isRequired == 1">*</strong><span>{{
										item.reserveName }}：</span></div>
							</td>
							...
							<td v-else-if="item.reserveType == 5">
								<el-input id="pre_tag_input" v-model="item.reserveValue"
									:placeholder="`请输入${item.reserveName}`" type="text" :maxlength="30"
									clearable :key="index" @input="preMsgErr.index = -1">
								</el-input>
							</td>
							<td v-else>
								<el-input id="pre_tag_input" v-model="item.reserveValue"
			...
			<div class="receive_info_title">
				<div class="line"></div>
				<span>{{ L['商品清单'] }}</span>
				<div class="add_ress" v-if="pre_params.data.ifcart == 'true'" @click="goCart">{{ L['返回修改购物车'] }}
				</div>
			</div>
			<div class="confirm_title_info">
				<div class="shop">店铺宝贝</div>
			...
			<div v-for="(storeItem, index) in goods_data.data" :key="index" class="store_item">
				<div class="good_container">
					<div class="store_name">
						<span>店铺：{{ storeItem.storeName }}</span>
					</div>
					<div v-for="(goodItem, index) in storeItem.productList" :key="index"
						...
						<div class="good_price">¥<span>{{ goodItem.price }}</span></div>
						<div class="num"><span>x</span>{{ goodItem.buyNum }}</div>
						<div class="subtotal">¥{{(Number(goodItem.buyNum)*goodItem.price).toFixed(2)}}</div>
					</div>
					<div v-for="(sendItem, index) in storeItem.sendProductList" :key="index"
						...
						<div class="good_info flex_row_start_center">
							...
							<div class="good_info_text">
								<div class="good_name overflow_ellipsis_two"><span class="giftProduct">{{
										L['赠品'] }}</span>{{ sendItem.goodsName }}</div>
				...
				<div class="additional">
					<div class="leaving_a_message">
						...
						<div class="store_coupon_con flex_row_start_center more_height">
							<el-input maxlength="100" v-model="storeItem.remark" type="textarea"
								:placeholder="L['选填，给卖家留言备注']" show-word-limit resize="none" rows="2" />
						</div>
					</div>
					<div class="discount_box">
						<div class="discount">
							...
							<div class="store_coupon_con flex_row_start_center">
								...
								<el-select v-else class="voice_select" v-model="couponCode[index]"
									@change="changeCoupon(storeItem, index)" placeholder="请选择">
									<el-option :value="L['不使用优惠券']">{{ L['不使用优惠券'] }}</el-option>
									<el-option v-for="(couponItem, index) in storeItem.availableCouponList"
										:key="index"
						...
						<div class="settle_info">
							...
							<div class="shipping">
								<span>{{ L['运费'] }}：</span>
								<span class="red" v-if="storeItem.expressFee">+￥{{ storeItem.expressFee
									}}</span>
					...
					<div class="invoice_con flex">
						<div class="no_select_invoice flex_row_center_center">
							...
							<div class="invoice_info" v-if="select_invoice_info.invoiceId">
								...
								<span @click="showVoiceDialog" class="choose">{{ L['重新选择'] }}</span>
							</div>
						</div>
					</div>
				</div>
			...
			<div>
				<div class="receive_info_title">
					<div class="line"></div>
					<span>{{ L['使用优惠券/积分抵用'] }}</span>
				</div>
				<div class="coupon_box">
					<div class="coupon_info">
						<div class="coupon_tit">
							<p class="flex_row_center_center" :class="favourable_show==0?'selected_p':''"
								@click="favourable(0)">平台优惠券</p>
							<p class="flex_row_center_center" :class="favourable_show==1?'selected_p':''"
								@click="favourable(1)">积分抵扣</p>
						</div>
						<div class="paging flex_row_end_center"
							v-if="order_settle_info.platformCouponList.length>8">
							<img src="../../assets/previous_page.png" @mouseover="previous(0)" alt=""
								v-if="!previous_page">
							<img src="../../assets/previous_page_xuan.png" @mouseout="previou(0)" @click="upper"
								alt="" v-if="previous_page">
							<span class="page flex_row_center_center">{{pagesize}}</span>
							<span class="xie">/</span>
							<span class="total flex_row_center_center">{{total}}</span>
							<img src="../../assets/next_page.png" alt="" @mouseover="previous(1)"
								v-if="!next_page">
							<img src="../../assets/next_page_xuan.png" alt="" @mouseout="previou(1)"
								@click="lower" v-if="next_page">
						</div>
						<div class="coupon_list" v-if="favourable_show==0">
							<template v-for="(item,index) in order_settle_info.platformCouponList" :key="index">
								<div class="coupon_cont flex_row_between_center"
									v-if="item.checked==false&&index+1>=rise&&index+1<=end"
									@click="platformDiscount(item.couponCode)">
									<div class="coupons">
										满减券
									</div>
									<div class="coupon_cont_box flex_column_between_start">
										<div class="money"><span>￥</span>{{item.discount}}</div>
										<div class="full_minus flex_row_center_center">{{item.content}}</div>
										<div class="coupon_time">{{item.useTime}}</div>
										<div class="user_rule"><span>使用规则：</span>{{item.description}}</div>
									</div>
									<img src="../../assets/coupon.png" alt="" class="img_coupon">
								</div>
								<div class="coupon_cont flex_row_between_center coupon_contone"
									v-if="item.checked==true&&index+1>=rise&&index+1<=end"
									@click="platformDiscount('不使用优惠券')">
									<div class="coupons coupons_one">
										满减券
									</div>
									<div class="coupon_cont_box flex_column_between_start">
										<div class="money cp_chosen"><span>￥</span>{{item.discount}}</div>
										<div class="full_minus flex_row_center_center">{{item.content}}</div>
										<div class="coupon_time">{{item.useTime}}</div>
										<div class="user_rule"><span>使用规则：</span>{{item.description}}</div>
									</div>
						...
						<div class="avali" v-if="favourable_show==1">
							<section v-if="allData.data.integralList && allData.data.integralList.length">
								<div class="int_avalible" @click="showInt" v-show="chosenInt == 0">
									<span>{{ L['可用积分'] }}：</span>
								...
								<div class="int_already" v-show="chosenInt > 0">
									...
									<div class="reChoose" @click="showInt">{{ L['重新选择'] }}</div>
								</div>
							</section>
							<section v-else>
								<div class="int_fail" v-if="Number(allData.data.totalAmount) < intRuleList[1]">
								...
								<div class="int_fail" v-else>
									{{ L['当前积分'] }}：{{ allData.data.memberIntegral }}，{{ L['满'] }}{{
									allData.data.integralScale
									}}{{ L['可用'] }}
								</div>
							</section>
						</div>
						<div class="purpose"
							v-if="chosenInt>0||(platformCouponCode!=''&&platformCouponCode!='不使用优惠券')">
							<span class="purpose_one">金额抵用</span>
							<span class="purpose_money">￥{{order_settle_info.totalDiscount}}</span>
							<span class="purpose_discount"
								v-if="platformCouponCode!=''&&platformCouponCode!='不使用优惠券'">使用优惠券<span>1</span>张，优惠{{default_plat_coupo.data.discount}}元</span>
							<span class="purpose_discount"
								v-if="chosenInt>0&&(platformCouponCode!=''&&platformCouponCode!='不使用优惠券')">；</span>
							<span v-if="chosenInt>0" class="purpose_fe">使用积分{{chosenInt}}分，抵扣{{ chosenInt /
								allData.data.integralScale }}元</span>
						</div>
					</div>
				</div>
				<div class="xian">
		...
		<div class="order_total_settle flex_column_between_end">
			<div class="order_price">
				...
				<span>¥ {{
					(Number(order_settle_info.totalAmount)+Number(order_settle_info.totalDiscount)).toFixed(2)
					}}</span>
			</div>
			<div class="order_price" v-if="order_settle_info.totalDiscount>0">
				<span>{{ L['共优惠'] }}：</span>
				<span>-&nbsp;¥ {{ order_settle_info.totalDiscount }}</span>
			</div>
			<div class="order_price">
				<span>{{ L['应付金额'] }}：</span>
				<span class="order_price_money">¥ <span>{{ order_settle_info.totalAmount }}</span></span>
			</div>
			<div class="order_price">
				<span>{{ L['送货至'] }}：</span>
				<span>
					<span>{{ orderAddress.data.addressAll ? orderAddress.data.addressAll : '--' }}{{
						orderAddress.data.detailAddress }}</span>
					<span></span>
					<span class="order_Name">收货人：{{ orderAddress.data.memberName ? orderAddress.data.memberName :
						'--' }}</span>
					<span class="order_mobile">{{ orderAddress.data.telMobile }}</span>
				</span>
			</div>
		</div>
		<div class="gopuy_box flex_row_end_center">
			<div class="gopuy flex_row_center_center" @click="checkOrder" style="cursor: pointer;">{{ L['提交订单'] }}
			</div>
		</div>
		...
		<el-dialog title="发票信息" v-model="edit_voice_dialog" customClass="voice_width" :before-close="handleClose"
			...
			<div v-if="!add_voice_content" class="voice_list">
				<div v-for="(voice, index) in invoice_list.data" :key="index" :title="voice.invoiceTitle"
					...
					<span>{{ voice.invoiceTitle }}</span> <span v-if="voice.isDefault">{{ L['默认'] }}</span>
							...
							<div class="right_info flex_column_center_start">
								<div class="item_select flex">
									<div @click="changeInvoiceContent(1)"
										...
										{{ L['商品明细'] }}
										<img v-if="invoice_info.data.invoiceContent == 1"
											:src="invoice_selected" alt />
									</div>
									<div @click="changeInvoiceContent(2)"
										:class="{ select_btn: true, pointer: true, active: invoice_info.data.invoiceContent == 2 }">
										{{ L['商品类别'] }}
										<img v-if="invoice_info.data.invoiceContent == 2"
											:src="invoice_selected" alt />
									</div>
								</div>
								<p>
									{{ invoice_info.data.invoiceContent == 1 ?
									L['发票内容将显示商品名称与价格信息，发票金额为实际支付金额，不含优惠等扣减金额'] :
									L['发票内容将显示本单商品所属类别及价格信息，发票金额为实际支付金额，不含优惠等扣减金额。']
									}}
								</p>
							</div>
						</div>
							...
							<div class="right_info flex_column_center_start">
								<div class="item_select flex">
									<div @click="changeInvoiceTitleType(1)"
										...
										个人
										<img v-if="invoice_info.data.titleType == 1" :src="invoice_selected"
											alt />
									</div>
									<div @click="changeInvoiceTitleType(2)"
										:class="{ select_btn: true, pointer: true, active: invoice_info.data.titleType == 2 }">
										公司
										<img v-if="invoice_info.data.titleType == 2" :src="invoice_selected"
											alt />
									</div>
								</div>
								<el-input v-if="invoice_info.data.titleType == 1" class="input title"
									size="small" :placeholder="L['请在此填写发票抬头']"
									v-model="invoice_info.data.invoiceTitle" clearable maxlength="30">
								</el-input>
							</div>
						</div>
						<!-- 发票类型： -->
							...
							<div class="right_info flex_column_center_start">
								<div class="item_select flex">
									<div @click="changeInvoiceType(1)"
										...
										{{ L['普通发票'] }}
										<img v-if="invoice_info.data.invoiceType == 1" :src="invoice_selected"
											alt />
									</div>
									<div @click="changeInvoiceType(2)"
										...
										{{ L['增值税专用发票'] }}
										<img v-if="invoice_info.data.invoiceType == 2" :src="invoice_selected"
											alt />
									</div>
								</div>
						...
						<div class="item flex" v-if="invoice_info.data.titleType == 2">
							...
							<div class="right_info flex_column_center_start">
								<el-input class="input nomartop" size="small" :placeholder="L['请在此填写单位名称']"
									v-model="invoice_info.data.companyName" clearable maxlength="30">
								</el-input>
							</div>
						</div>
						<!-- 税号： -->
							...
							<div class="item flex">
								...
								<div class="right_info flex_column_center_start">
									<el-input class="input" size="small" :placeholder="L['请输入注册地址']"
										v-model="invoice_info.data.registerAddr" clearable maxlength="100"
							...
							<div class="item flex">
								...
								<div class="right_info flex_column_center_start">
									<el-input class="input" size="small" :placeholder="L['请输入注册电话']"
										v-model="invoice_info.data.registerPhone" clearable maxlength="11"
										@blur="invoiceCheck('registerPhone')" type="number">
									</el-input>
								</div>
							</div>
							<div class="item flex">
							...
							<div class="item flex">
								...
								<div class="right_info flex_column_center_start">
									<el-input class="input" size="small" :placeholder="L['请输入银行账户']"
										...
										@blur="invoiceCheck('bankAccount')" type="number">
									</el-input>
								</div>
							</div>
							<div class="item flex">
							...
							<div class="item flex">
								...
								<div class="right_info flex_column_center_start">
									<el-input class="input" size="small" placeholder="请输入收票人电话"
										v-model="invoice_info.data.receiverMobile" clearable maxlength="11"
										@blur="invoiceCheck('receiverMobile')" type="number"></el-input>
								</div>
							</div>
							<div class="item flex">
								...
								<div class="right_info flex_column_center_start">
									<el-input class="input" size="small" placeholder="请输入收票人地址"
										v-model="invoice_info.data.receiverAddress" clearable maxlength="100"
						...
						<div class="item flex" v-if="invoice_info.data.invoiceType != 2">
							...
							<div class="right_info flex_column_center_start input_email">
								<el-input class="input nomartop" size="small" :placeholder="L['请输入收票邮箱']"
									v-model="invoice_info.data.receiverEmail" clearable maxlength="100"
									show-word-limit @blur="invoiceCheck('receiverEmail')">
								</el-input>
							</div>
						</div>
		...
		<el-dialog :title="no_good_info.data.stateValue" v-model="out_stock_dialog" customClass="out_store_width"
			...
			<div class="out_stock_dialog">
				<div v-for="(outStockItem, index) in no_good_info.data.productList" :key="index"
			...
			<div class="btn_con flex_row_center_center"
				v-if="no_good_info.data.state == 2 || no_good_info.data.state == 4">
			...
			<div class="btn_con flex_row_center_center" v-if="no_good_info.data.state == 7">
				<div class="return" @click="returnLastPage">{{ L['返回'] }}</div>
				<div class="remove" @click="goNext">继续下单</div>
			</div>
		...
		<!-- 积分抵扣弹框 -->
		<el-dialog title="使用积分" v-model="showIntList" width="550px" lock-scroll="false" top="9%">
			...
			<div class="use_integral_list_order">
				<div class="integral_item_order" v-for="(item, integralIndex) in allData.data.integralList"
					...
					<span>{{ L['抵扣'] }}<font color="#e2231a">¥{{ item / allData.data.integralScale }}</font>{{
						L['使用']
						}}
					...
					<div>
						<input type="radio" :value="item" :id="integralIndex" name="integral"
							:checked="tmpInt == item">
		...
		<!-- 积分规则框 -->
		<el-dialog title="使用规则" v-model="showIntRule" width="390px" lock-scroll="false" top="10%">
			<div class="intRule_con">
				<div class="int_rule_item">{{ L['订单大于等于'] }}<font color="#e2231a">{{ intRuleList[1] }}{{ L['元'] }}
					</font>
					{{ L['可用'] }};
				</div>
...
<script>
	...
	export default {
		...
		setup() {
			...
			const favourable_show = ref(0);//选择优惠券和积分
			const default_plat_coupo = reactive({ data: {} });//选择优惠券和积分
					...
					if (res.state == 200) {
						if (type == 1) {
							...
							goods_data.data.map((item, index) => {
								...
								if (default_coupon.length > 0) {
									item.couponCode = default_coupon[0].couponCode;
									couponCode.value[index] = default_coupon[0].couponCode ? default_coupon[0].couponCode : L["不使用优惠券"]
								} else {
						...
						total.value = Math.ceil(order_settle_info.platformCouponList.length / 8)
						let default_plat_cou = order_settle_info.platformCouponList.filter((coupon) => coupon.checked)
						let default_plat_coupons = JSON.parse(JSON.stringify(default_plat_cou))
						if (default_plat_coupons.length) {
							default_plat_coupo.data = default_plat_coupons[0]
						}
						order_settle_info.totalDiscount = res.data.totalDiscount;

						//处理平台优惠券
						let default_plat_coupon = order_settle_info.platformCouponList.filter((coupon) => coupon.checked)
						if (default_plat_coupon.length > 0) {
							platformCouponCode.value = default_plat_coupon[0].couponCode;
						} else {
			...
			//选择优惠券和积分
			const favourable = index => {
				favourable_show.value = index;
			};
			//确认修改地址
			...
			const confirmAddVoice = () => {
				...
				if (invoice_info.data.titleType == 1) {
					...
					if (invoice_info.data.companyName == "") {
						...
						if (proxy.$checkPhone(invoice_info.data.registerPhone, '请正确填写注册电话') != true) {
							ElMessage(proxy.$checkPhone(invoice_info.data.registerPhone, '请正确填写注册电话'))
							return
						}
						//-----
						...
						if (proxy.$checkPhone(invoice_info.data.receiverMobile, '请正确填写收票人电话') != true) {
							ElMessage(proxy.$checkPhone(invoice_info.data.receiverMobile, '请正确填写收票人电话'))
							return
						}
			...
			const checkOrder = () => {
				...
				if (pre_params.data.ifcart == "true") {
					...
					let { price, promotionId, promotionType } = goods_data.data[0].productList[0]
					...
					.then(res => {
						if (res.state == 267) {
							...
							if (state == 7 || state == 4 || state == 3 || state == 2 || state == 1) {
								...
								proxy.$confirm('商品促销信息发生变化，请返回后重新下单', '商品促销信息发生变化', {
			...
			const showInt = () => {
				showIntList.value = true
				tmpInt.value = chosenInt.value
			}
