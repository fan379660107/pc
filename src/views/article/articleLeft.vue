				...
				<div class="article_content">
					<ul class="nch_sidebar_article_class">
						<li v-for="(item,index) in data.allCategory" :key="item.categoryId"
							...
							<router-link :to="{ path: 'article', query: { categoryId: item.categoryId }}"
								replace>
								<a
									@click="getArticalDetail(item.categoryName,index,item.categoryId)">{{item.categoryName}}></a>
							</router-link>
						</li>
					</ul>
				</div>
			...
			<div class="nch_module nch_module_style02">
				...
				<div class="article_content">
					<ul class="nch_sidebar_article_list">
						<li v-for="(item,index) in data.cate" :key="index"
							@click="toDetail(item.articleId,index)">
							<div class="sortes">
								<i></i>
								<a
									:class="data.articleIndex==index?'help_name_other':'help_name'">{{item.title}}</a>
							</div>
							<p class="help_time">
	...
	export default {
		...
		setup(props, { emit }) {
			...
			const getArticalList = () => {
				...
				proxy.$get("v3/cms/front/article/helpList", params1).then(res => {
					if (res.state === 200) {
						data.allCategory = res.data;
						if (!route.query.categoryId && !route.query.articleId) {
							//获取服务中心数据
							if (res.data.length > 0) {
								emit("getArticalDetail", res.data[0].categoryName, res.data[0].categoryId, true)
							}
						} else {
							for (let i = 0; i < data.allCategory.length; i++) {
								if (data.allCategory[i].categoryId == route.query.categoryId) {
									...
									let idx = data.allCategory[i].articleList.findIndex(x => x.articleId == route.query.articleId)
									if (idx > -1) {
										data.curindex = i;
										emit("settitle", data.allCategory[i].categoryName);
									}
			...
			const getList = () => {
				const params = {};
				params.size = 6;
				proxy.$get("v3/cms/front/article/articleList", params).then(res => {
					if (res.state == 200) {
						data.cate = res.data;
					}
				});
