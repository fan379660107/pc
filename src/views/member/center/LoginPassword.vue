...
<template>
  <div class="sld_login_password_mange">
    ...
    <div class="container">
      <div class="title">{{memberInfo.data.hasLoginPassword?'修改':'设置'}}登录密码</div>
      <div class="mange_con">
        <div class="without_phone_tip" v-if="!memberInfo.data.memberMobile">请先绑定手机号，再进行登陆密码操作!</div>
        <!-- 设置登录密码 start -->
        <template v-if="!memberInfo.data.hasLoginPassword">
          ...
          <el-input type="password" class="password input" v-model="password" placeholder="请输入密码"></el-input>
          <el-input type="password" class="password input" v-model="confirm_password" placeholder="请再次输入密码"></el-input>
          <div class="error_tip">
            <span v-if="errorMsg" style="color: #e1251b;font-size: 14px" class="iconfont icon-jubao"></span>
            {{errorMsg}}
          </div>
          <div class="next flex_row_center_center pointer" @click="next">设置密码</div>
        </template>
        <!-- 设置登录密码 end -->
        <!-- 修改登录密码 start -->
        <template v-if="memberInfo.data.hasLoginPassword">
          <el-input type="password" class="password input" v-model="old_password" placeholder="请输入原密码"></el-input>
          <el-input type="password" class="password input" v-model="password" placeholder="请输入新密码"></el-input>
          <el-input type="password" class="password input" v-model="confirm_password" placeholder="请再次输入新密码"></el-input>
          <div class="error_tip">
            <span v-if="errorMsg" style="color: #e1251b;font-size: 14px" class="iconfont icon-jubao"></span>
            {{errorMsg}}
          </div>
          <div class="next flex_row_center_center pointer" @click="next">修改密码</div>
        </template>
        <!-- 修改登录密码 end -->
      </div>
    ...
    setup() {
      ...
      const next = () => {
        ...
        if (memberInfo.data.hasLoginPassword) {
          ...
          //设置登录密码
          url = "v3/member/front/memberPassword/addLoginPwd";
          param.memberMobile = memberInfo.data.memberMobile;
          param.verifyCode = sms_code.value;
          param.loginPwd = old_password.value;
        }
