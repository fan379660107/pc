 ...
 * @LastEditTime: 2021-01-15 16:33:06
...
<template>
  <div class="sld_phone_mange">
    ...
    <div class="container">
      ...
      <div class="mange_con">
        <span class="current">{{L['当前手机号']}} {{memberInfo.data.memberMobile}}</span>
        ...
        <el-input v-if="step==2" class="password input" v-model="new_mobile" :placeholder="L['请输入新手机号']" type="number">
        </el-input>
        ...
        <div class="next flex_row_center_center" @click="next">{{step==1?L['下一步']:L['确定']}}</div>
      </div>
  ...
  export default {
    ...
    setup() {
      ...
      const getSmsCode = () => {
        ...
        param.verifyAddr =
          step.value == 1 ? memberInfo.data.memberMobile : new_mobile.value;
      ...
      const next = () => {
        ...
        if (step.value == 1) {
          ...
          proxy
            .$post("v3/member/front/memberPassword/verifyOldMobile", {
              memberMobile: memberInfo.data.memberMobile,
          ...
          proxy
            .$post("v3/member/front/memberPassword/editMobile", {
              memberMobile: new_mobile.value,
            ...
            .then(res => {
              if (res.state == 200) {
                clearTimeout(timeOutId.value);
                //更新个人信息
                memberInfo.data.memberMobile = new_mobile.value;
