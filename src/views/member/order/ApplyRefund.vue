...
<template>
	<div class="sld_apply_refund">
		...
		<!-- 退款商品信息 start -->
		<refund-goods :refundOrderDetail="refundOrderDetail.data"
			v-if="batchGoods.data && batchGoods.data.length == 0">
		</refund-goods>
		<!-- 退款商品信息 end -->
		<div v-if="batchGoods.data && batchGoods.data.length > 0">
			<refund-goods :refundOrderDetail="batchGoodsItem"
				v-for="(batchGoodsItem, batchGoodsIndex) in batchGoods.data" :key="batchGoodsIndex"></refund-goods>
		</div>
		<!-- 退款详细信息 start -->
          ...
          ">
				...
				<div v-else class="flex_row_start_center">
					...
					<div class="edit_refund_amount flex_row_start_center" v-if="editAmountShow">
						<el-input v-model="curEditRefundAmount" :placeholder="L['请输入退款金额']" type="number"
							class="amount_input" @blur="handleBlur">
						</el-input>
          ...
          ">
			...
			<div class="cargo_status flex_row_start_center marginBottom20"
				...
				<el-radio-group class="cargo_status_btn flex_row_start_center" v-model="curCargoStatus"
					...
					<el-radio :label="cargoStatusItem.value"
						v-for="(cargoStatusItem, cargoStatusIndex) in cargoStatuslList.data"
			...
			<div class="refund_reasons flex_row_start_center marginBottom20">
				<div class="refund_title"><span>*</span>{{ curApplyType=="0" ? L["退款原因"] : L["退货原因"] }}：</div>
				<el-select :placeholder="curApplyType=='0' ? L['请选择退款原因'] : L['请选择退货原因']"
					v-if="cancelList.data && cancelList.data.length > 0" @change="changeCancel($event)"
					v-model="curCancelReason" id="elSelect">
					<el-option v-for="cancelItem in cancelList.data" :key="cancelItem.reasonId"
						:label="cancelItem.content" :value="cancelItem.reasonId">
					</el-option>
				</el-select>
			</div>
			<!-- 退款原因 end -->
			...
			<div class="upload_voucher flex_row_start_start marginBottom20">
				...
				<div>
					<div class="upload_count">总共上传<span style="color: red;">{{upLoadFile.data.length}}</span>/5张图片
					</div>
					<div class="upload">
						<el-upload :action="uploadImgUrl" list-type="picture-card"
							:on-preview="handlePictureCardPreview" :on-remove="handleRemove" limit="5"
							:data="upload_data" name="file" :on-success="uploadImg" :on-exceed="exceedHandle"
							id="elUpload" accept="image/*">
							<template #default>
			...
			<div class="batch_selModel">
				<div class="batch_default">
					<div class="batch_default_pre flex_row_start_center">
						...
						<div :style="{ backgroundImage: 'url(' + defaultGoods.data.productImage + ')' }"
							class="batch_default_img"></div>
						<div class="batch_default_des">
							<p>{{ defaultGoods.data.goodsName }}</p>
							<p>{{ defaultGoods.data.specValues }}</p>
						</div>
						<el-input-number v-model="defaultGoods.data.editNum" @change="handleEditDefaultNum"
							:min="1"
							:max="defaultGoods.data.productNum > 999 ? 999 : defaultGoods.data.productNum"
							label="描述文字" :class="edit_batch_num" :disabled="orderState==20"></el-input-number>
					</div>
				</div>
				<div class="batch_default">
					<div class="batch_default_pre flex_row_start_center"
						...
						<img src="@/assets/order/sel.png" alt="" class="batch_default_sel"
							v-if="batchGoodsItem.sel" @click="selGoods(batchGoodsItem.productId)" />
						<img src="@/assets/order/no_sel.png" alt="" class="batch_default_sel"
							v-if="!batchGoodsItem.sel" @click="selGoods(batchGoodsItem.productId)" />
						<div :style="{ backgroundImage: 'url(' + batchGoodsItem.productImage + ')' }"
							class="batch_default_img"></div>
						<div class="batch_default_des">
		...
		setup() {
			...
			const submitApplyRefund = () => {
				if (curCancelReason.value == "" && curApplyType.value == '0') {
					...
					let params = {
						orderSn: refundOrderDetail.data.orderSn,
						afsType: curApplyType.value == "0" ? 3 : 1, //申请类型（售后服务单类型，1-退货退款单（需关联处理退款金额），2-换货单，3-仅退款单）
