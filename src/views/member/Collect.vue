<template>
  <div class="sld_collect_wrapper">
    <MemberTitle :memberTitle="collectIndex==1 ?  L['收藏的商品'] : L['收藏的店铺']" style="padding-left:20px"></MemberTitle>
      ...
      <div class="sld_follow_m">
        ...
        <div class="sld_h3_wrap clearfix">
          ...
          <div class="sld_option fr" v-show="1===collectIndex&&collectGoodsData.list.length">
            ...
            <button class="sld_option_btn fr" @click="optAct = !optAct">{{L['管理']}}</button>
          </div>
        </div>
        ...
        <div>
          ...
          <div class="fav_goods_list" v-show="1===collectIndex">
            ...
            <ul class="clearfix">
              <li class="sld_follow_goods" v-for="(item,index) in collectGoodsData.list" :key="index">
                <div class="cg_img">
                  <router-link :to="`/goods/detail?productId=${item.productId}`">
                    <!-- <img class="lazy" alt="" :src="item.productImage" style="display: inline;"> -->
                    <coverImage :src="item.productImage" class="lazy" />
                  </router-link>
                </div>
                <p class="cg_price clearfix">
                  <span class="fl">￥<strong>{{item.productPrice}}</strong></span>
                ...
                <div class="cg_btns">
                  <router-link :to="`/store/index?vid=${item.storeId}`" class="govendor">{{L['进入店铺']}}</router-link>
                  <a class="govendor" @click="addCart(item)">{{L['加入购物车']}}</a>
                </div>
              ...
              <SldCommonEmpty v-show="!collectGoodsData.list.length" totalWidth="1003" tip="暂无收藏商品" />
            </ul>
            <!-- 分页 -->
          ...
          <div class="fav_store_list" v-show="2===collectIndex">
            <div class="fav_store_type">
              <span :class="1===collectStoreIndex?'on':''" @click="switchStoreType(1)">{{L['特别关注']}}</span>
              <span :class="2===collectStoreIndex?'on':''" @click="switchStoreType(2)">{{L['普通关注']}}</span>
            </div>
            <!-- 特殊关注的店铺 -->
            <div v-if="1===collectStoreIndex">
              ...
              <SldCommonEmpty v-show="!collectLength.specialLength" totalWidth="1003" tip="暂无收藏店铺" />
            </div>
            <!-- 普通关注的店铺 -->
            <div v-else>
              ...
              <SldCommonEmpty v-show="!collectLength.commonLength" totalWidth="1003" tip="暂无收藏店铺" />
            </div>
          </div>
        </div>
      </div>
  ...
  export default {
    ...
    setup() {
      ...
      const getInitData = (params) => {
        proxy.$get('v3/member/front/followProduct/list', params).then(res => {//获取收藏商品信息
          if (res.state == 200) {
      ...
      const getStoreData = () => {
        proxy.$get('v3/member/front/followStore/list').then(res => {//获取收藏店铺信息
          if (res.state == 200) {
            specialFollowStore.value = res.data.storeList.filter(item => item.isTop == 1)
            commonFollowStore.value = res.data.storeList.filter(item => item.isTop == 0)
