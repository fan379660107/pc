            ...
            <div class="flex_row_start_center">
                <div class="address_item" v-if="default_address.data.length>0"
                    ...
                    <div class="defalut_edit flex_row_between_center">
                        ...
                        <div class="oprate">
                            <i class="iconfont icon-bianji pointer" @click="editAddress(default_address.data[0])"></i>
                        </div>
                    </div>
                    <div class="name_phone">
                        <span class="name">{{default_address.data[0].memberName}}</span>
                        <span class="phone">{{default_address.data[0].telMobile}}</span>
                    </div>
                    <div class="add_info">{{default_address.data[0].addressAll}}</div>
                    <div class="add_detail overflow_ellipsis_two">{{default_address.data[0].detailAddress}}</div>
                </div>
                <div class="add_address flex_column_center_center pointer" @click="addAddress">
                    <i class="iconfont icon-xinzengdizhi"></i>
                    <p>{{L['新增地址']}}</p>
                </div>
            </div>
        </div>
        <div class="address_list_con">
            <p class="title mange">{{L['全部地址']}}</p>
            <div class="container flex_row_start_center">
                <div v-for="(addressItem,index) in address_list.data" :key="index" class="address_item"
                    ...
                    <div class="defalut_edit flex_row_between_center">
                        <span class="red">{{addressItem.isDefault==1?'默认地址':' '}}</span>
                        <div class="oprate">
                            <i class="iconfont icon-duihao1 pointer" @click="setDefault(addressItem.addressId)"
                                v-if="addressItem.isDefault!=1" :title="L['设置为默认']"></i>
                            <i class="iconfont icon-bianji pointer" @click="editAddress(addressItem)"
                                :title="L['编辑地址']"></i>
                            <i class="iconfont icon-lajitong pointer" @click="delAddress(addressItem.addressId)"
                                :title="L['删除地址']"></i>
                        </div>
                    </div>
                    <div class="name_phone">
                        <span class="name">{{addressItem.memberName}}</span>
                        <span class="phone">{{addressItem.telMobile}}</span>
                    </div>
                    <div class="add_info">{{addressItem.addressAll}}</div>
                    <div class="add_detail overflow_ellipsis_two" :title="addressItem.detailAddress">
                        {{addressItem.detailAddress}}</div>
                </div>
            </div>
        ...
        setup() {
            ...
            const delAddress = (addressId) => {
                ...
                ElMessageBox.confirm('确定删除该地址?', '提示', {
                    ...
                    proxy.$post("v3/member/front/memberAddress/del", {
