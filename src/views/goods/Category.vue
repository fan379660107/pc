                        ...
                        <div class="cat_wrap_dd">
                            <div class="cat_dd_item" v-for="(item2,cateIndex) in childCate.data" :key="cateIndex"
                                ...
                                <div class="fl cat_left" :style="{color:color,cursor:'pointer'}"
                                    @click="toGoodsList(item2)">
                                    {{item2.categoryName}} >
                                </div>
                                <div class="cat_add_right">
                                    <span v-for="(item3,index) in item2.children" @click="toGoodsList(item3)"
    ...
    export default {
        ...
        setup() {
            ...
            const getInitData = (index, params) => {
                proxy.$get('v3/goods/front/goods/category/topCategory', params).then(res => {
            ...
            const getChild = (categoryId1, index) => {
                ...
                proxy.$get('v3/goods/front/goods/category/bottomCategory', { categoryId1 }).then(res => {
                    // navList.data[index].children = res.data
            ...
            window.onscroll = function () {
                ...
                // li.forEach((item, i) => {
                //     let categoryId = item.getAttribute("data-categoryId")
                //     if (scrollTop >= li[i].offsetTop) {
                //         indexNum.value = i

                //         if (!navList.data[i].children) {
                //             getChild(categoryId, i)
                //         }
                //     }
                // });
            }
            // window.addEventListener('mousewheel', () => {
            //     srollOrTap.value = 'scroll'
            // }, false)
            ...
            return { navList, noneUrl, indexNum, toGoodsList, L, firstLoading, isTop, toTop, isBottom, childCate, cateStyle };
        },
    };
