    ...
    <div class="sld_chatRightList" :style="'height:'+(clientHeight-180)+'px'">
        ...
        <div class="header">
            ...
            <div class="header_item" @click="select(3)" v-if="curStoreId!=0">
                <p :class="{on:selectFlag==3}">店铺推荐</p>
                            ...
                            <div :class="{brief:true,order_con:true,sld_hide:item.isFold,sld_show:!item.isFold}"
                                ...
                                <div class="text_con">
                                    ...
                                    <div class="bottom_con">
                                        <p>￥{{goodsItem.productShowPrice}}</p>
                                        <a @click="sendLink({item,goodsItem},'order')">发送链接
                                            ></a>
                                    </div>
                                </div>
                            </div>
                    ...
                    <div class="footPrint_item" v-for="(foot,footIdx) in footlist.list" :key="footIdx">
                        <div class="brief footPrint_con">
                            ...
                            <div class="text_con">
                                <p>{{foot.goodsName}}</p>
                                <div class="bottom_con">
                                    <p class="price">￥{{foot.productPrice}}</p>
                                    <a @click="sendLink(foot,'foot')">发送链接 ></a>
                                </div>
                            </div>
                        </div>
                    </div>
                    <loadingState v-if="loadState.orderState  == 'first_loading'||footlist.list.length > 0"
                        :state='loadState.orderState' />
            ...
            <el-scrollbar>
                <div class="storeRecom" v-infinite-scroll="load" infinite-scroll-immediate="false">
                    <div class="store_info">
                        <p class="store_name">店铺名称：<span>{{storeDetail.storeName}}</span></p>
                        <p class="store_level">店铺星级：
                            <el-rate disabled :colors="colors" score-template="{score}" v-model="score">
                            </el-rate>
                        </p>
                        <p class="store_contact">联系电话：<span>{{storeDetail.servicePhone}}</span></p>
                    </div>
                    ...
                    <div class="store_score">
                        ...
                        <p :class="{highScore:storeDetail.descriptionScore > 4}">
                            用户评价：<span>{{storeDetail.descriptionScore}}</span>&nbsp;&nbsp;&nbsp;<span
                                v-if="storeDetail.descriptionScore > 4">高</span>
                            <span v-else-if="storeDetail.descriptionScore < 2">低</span>
                            <span v-else>中</span>
                        </p>
                        <p :class="{highScore:storeDetail.deliverScore > 4}">
                            物流服务：<span>{{storeDetail.deliverScore}}</span>&nbsp;&nbsp;&nbsp;<span
                                v-if="storeDetail.deliverScore > 4">高</span>
                            <span v-else-if="storeDetail.deliverScore < 2">低</span>
                            <span v-else>中</span>
                        </p>
                        <p :class="{highScore:storeDetail.serviceScore > 4}">
                            售后服务：<span>{{storeDetail.serviceScore}}</span>&nbsp;&nbsp;&nbsp;<span
                                v-if="storeDetail.serviceScore > 4">高</span>
                            <span v-else-if="storeDetail.serviceScore < 2">低</span>
                            <span v-else>中</span>
                        </p>
                    </div>
                    <div class="recomList">
                        ...
                        <div class="recom_item" v-for="(recom,index) in recomList.list" :key="index">
                            <div class="brief recom_con">
                                ...
                                <div class="text_con">
                                    <p>{{recom.goodsName}}</p>
                                    <div class="bottom_con">
                                        <p class="price">￥{{recom.goodsPrice}}</p>
                                        <a @click="sendLink(recom,'recom')">发送链接 ></a>
                                    </div>
                                </div>
                            </div>
                        </div>
