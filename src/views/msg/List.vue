...
<template>
  <div class="sld_msg_list">
    ...
    <div class="msg_con" v-if="noticeList.data && noticeList.data.length > 0">
      <div class="msg_con_top flex_row_between_center">
        <el-checkbox v-model="checkedAll" @change="checkMsgAll">{{ L["全选"] }}</el-checkbox>
          ...
          <div class="msg_pre">
            <div class="msg_pre_title flex_row_between_center">
              ...
              <el-popconfirm :confirmButtonText="L['确定']" :cancelButtonText="L['取消']" icon="el-icon-info"
                ...
                <template #reference><i class="iconfont icon-ziyuan51 pre_title_right cursor_pointer"></i></template>
              </el-popconfirm>
            </div>
            <!-- 	消息状态：0--未读，1--已读，2--删除 -->
    ...
    <SldCommonEmpty v-if="!noticeList.data || (noticeList.data && noticeList.data.length == 0)" totalWidth="900"
      totalHeight="300" style="color:#666" :tip="L['暂时没有消息！']" />
  </div>
</template>
...
<script>
  ...
  export default {
    ...
    setup() {
      ...
      const listTitle = ref('消息列表')
      const listTitleMap = ref(new Map())
      listTitleMap.value.set('msg_all', '消息列表')
      listTitleMap.value.set('system_news', '系统消息')
      listTitleMap.value.set('order_news', '订单消息')
      listTitleMap.value.set('assets_news', '资产消息')
      listTitleMap.value.set('after_sale_news', '售后消息')
      listTitleMap.value.set('appointment_news', '预约提醒')
      onMounted(() => {
      ...
      watch(() => curTplTypeCode.value, (nv, ov) => {
        if (nv != ov) {
      ...
      const delMsg = (receiveId) => {
        ...
        if (receiveIds.value) {
          ...
          proxy.$confirm(L['确定删除这些消息吗'], L['提示'], {
      ...
      const goMsgDetail = (noticeItem) => {
        ...
        let curNotice = []; //当前消息
        curNotice = noticeList.data.filter((item => item.receiveId == noticeItem.receiveId))
        ...
        switch (msgLinkInfo.type) {
          //仅退款
          case 'order_news':
            if (msgLinkInfo.orderType && msgLinkInfo.orderType == 'integral_order') {
